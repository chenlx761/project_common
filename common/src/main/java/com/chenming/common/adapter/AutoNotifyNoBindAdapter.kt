 ...
 * 监听数据变化和有空布局的adapter
 */
open abstract class AutoNotifyNoBindAdapter<T : RecyclerView.ViewHolder, K>() :
    ...
    private class WeakReferenceOnListChangedCallback<K : RecyclerView.ViewHolder, T> constructor(
        adapter: AutoNotifyNoBindAdapter<K, T>
    ) : OnListChangedCallback<ObservableList<T>>() {


        var adapterRef: WeakReference<AutoNotifyNoBindAdapter<K, T>> =
            WeakReference<AutoNotifyNoBindAdapter<K, T>>(adapter)
